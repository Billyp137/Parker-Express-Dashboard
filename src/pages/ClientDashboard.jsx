...
export default function ClientDashboard() {
  ...
  const filteredByRange = shipments.filter(s => {
        ...
        return s.dispatchDateRaw ? isAfter(new Date(s.dispatchDateRaw), cutoff) : true
  })
  ...
  const dailyData = (() => {
        ...
        filteredByRange.forEach(s => {
                if (s.dispatchDateRaw) {
                          const k = format(new Date(s.dispatchDateRaw), 'dd MMM')
  ...
  const carrierData = Object.entries(
        filteredByRange.reduce((acc, s) => {
                acc[s.carrier] = (acc[s.carrier] || 0) + 1; return acc
        }, {})
  ...
  const filteredShipments = filteredByRange.filter(s => {
        ...
        const matchQ = !q || s.trackingNumber?.toLowerCase().includes(q) ||
                s.recipient?.toLowerCase().includes(q) || s.reference?.toLowerCase().includes(q)
  ...
  const handleTrack = async () => {
        ...
        try {
                const res = await fetch(`/api/tracking?number=${encodeURIComponent(trackingInput.trim())}`, {
                          headers: { 'x-client-token': token }
                })
                if (!res.ok) {
                          const data = await res.json().catch(() => ({}))
                ...
                setTrackingError(err.message || 'Failed to fetch tracking info')
        } finally {
      ...
      if (!client) return (
            <div className="min-h-screen bg-dark flex items-center justify-center p-6">
                  <div className="text-center max-w-sm">
                          <Logo size="lg" className="justify-center mb-6" />
                          <div className="text-zinc-400 mb-2">Dashboard not found</div>div>
                          <div className="text-zinc-600 text-sm">This link may be invalid or expired. Contact Parker Express for a new link.</div>div>
                  </div>div>
            </div>div>
          )
                ...
                <div className="min-h-screen bg-dark">
                  {/* Header */}
                              ...
                              <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-4">
                                                    <div className="text-right hidden sm:block">
                                                                  <div className="text-xs text-zinc-500">Viewing dashboard for</div>div>
                                                                  <div className="text-sm font-semibold text-gold">{client.name}</div>div>
                                                    </div>div>
                                                    <button
                      ...
                      <main className="max-w-7xl mx-auto px-6 py-8">
                        ...
                        {/* Error banner */}
                        {error && (
                            <div className="mb-6 bg-red-950 border border-red-900 rounded-xl px-4 py-3 flex items-center gap-3 text-red-400 text-sm">
                                        <AlertCircle size={16} className="shrink-0" />
                                        <span>Could not load live data: <strong>{error}</strong>strong>. Contact Parker Express to check your API configuration.</span>span>
                            </div>div>
                              )}
                            ...
                            <div>
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
                                          {/* Daily bar chart */}
                                                      <div className="lg:col-span-2 bg-card border border-border rounded-xl p-5 animate-fade-up stagger-3">
                                                                      <div className="text-xs text-zinc-500 uppercase tracking-widest mb-4 font-medium">Shipments dispatched</div>div>
                                          ...
                                          {/* Status donut */}
                                                      <div className="bg-card border border-border rounded-xl p-5 animate-fade-up stagger-4">
                                                                      <div className="text-xs text-zinc-500 uppercase tracking-widest mb-4 font-medium">Status breakdown</div>div>
                                                                      <ResponsiveContainer width="100%" height={180}>
                                                                                        <PieChart>
                                                  ...
                                                  <div key={s.name} className="flex items-center justify-between text-xs">
                                                                        <div className="flex items-center gap-2">
                                                                                                ...
                                                                                                <span className="text-zinc-400">{s.name}</span>span>
                                                                        </div>div>
                                                                        <span className="text-white font-medium">{s.value}</span>span>
                                                  </div>div>
                                                ))}
                                                                      </div>div>
                                                      </div>div>
                              ...
                              {/* Carrier split */}
                                        <div className="bg-card border border-border rounded-xl p-5 animate-fade-up stagger-5">
                                                      <div className="text-xs text-zinc-500 uppercase tracking-widest mb-4 font-medium">Carrier split</div>div>
                                                      <div className="flex flex-col gap-3">
                                                        {carrierData.map(c => (
                                                <div key={c.name}>
                                                                    <div className="flex justify-between text-sm mb-1.5">
                                                                                          <span className="text-zinc-300">{c.name}</span>span>
                                                                                          <span className="text-zinc-500">{c.value} <span className="text-zinc-600">({Math.round(c.value / stats.total * 100)}%)</span>span></span>span>
                                                                    </div>div>
                                                                    <div className="h-1.5 bg-border rounded-full overflow-hidden">
                                                                                          <div className="h-full bg-gold rounded-full transition-all duration-700" style={{ width: `${c.value / stats.total * 100}%` }} />
                                                                    </div>div>
                                                </div>div>
                                              ))}
                                                      </div>div>
                        ...
                        {activeTab === 'Shipments' && (
                            <div>
                                        ...
                                        <div className="text-xs text-zinc-600 mb-3">{filteredShipments.length} shipments</div>div>
                                        <div className="bg-card border border-border rounded-xl overflow-hidden">
                                                      ...
                                                      <th key={h} className="text-left px-4 py-3 text-xs text-zinc-500 uppercase tracking-wider font-medium whitespace-nowrap">{h}</th>th>
                                                    ))}
                                                                                                              </tr>tr>
                                                                                          </thead>thead>
                                                                                        <tbody>
                                                    ...
                                                    <tr key={s.id || i} className="border-b border-border hover:bg-surface transition-colors">
                                                                            <td className="px-4 py-3 font-mono text-xs text-gold">{s.trackingNumber}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-500 text-xs">{s.reference}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-200 whitespace-nowrap">{s.recipient}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-400 whitespace-nowrap text-xs">{s.destination}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-500 text-xs whitespace-nowrap">{s.carrier}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-500 text-xs whitespace-nowrap">{s.dispatchDate}</td>td>
                                                                            <td className="px-4 py-3 text-zinc-500 text-xs whitespace-nowrap">{s.estDelivery}</td>td>
                                                                            <td className="px-4 py-3"><StatusPill status={s.status} /></td>td>
                                                    </tr>tr>
                                                  ))}
                                                                                          {filteredShipments.length === 0 && (
                                                    <tr><td colSpan={8} className="px-4 py-12 text-center text-zinc-600 text-sm">No shipments found</td>td></tr>tr>
                                                                                                            )}
                                                                                          </tbody>tbody>
                                                                      </table>table>
                                                      </div>div>
                            ...
                            <div className="max-w-2xl">
                              ...
                              {trackingResult && (
                                            <div className="bg-card border border-border rounded-xl p-6 animate-fade-up stagger-1">
                                                            <div className="flex items-start justify-between mb-6 pb-6 border-b border-border">
                                                                              <div>
                                                                                                  <div className="text-xs text-zinc-500 mb-1">Tracking number</div>div>
                                                                                                  <div className="font-mono text-gold font-medium">{trackingResult.trackingNumber}</div>div>
                                                                              </div>div>
                                                                              <StatusPill status={trackingResult.status} />
                                                            </div>div>
                                                            <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-border">
                                                                              <div><div className="text-xs text-zinc-500 mb-1">Carrier</div>div><div className="text-sm text-zinc-200">{trackingResult.carrier}</div>div></div>div>
                                                                              <div><div className="text-xs text-zinc-500 mb-1">Current status</div>div><div className="text-sm text-zinc-200">{trackingResult.status}</div>div></div>div>
                                                              {trackingResult.recipient && <div><div className="text-xs text-zinc-500 mb-1">Recipient</div>div><div className="text-sm text-zinc-200">{trackingResult.recipient}</div>div></div>div>}
                                                              {trackingResult.estDelivery && trackingResult.estDelivery !== '—' && <div><div className="text-xs text-zinc-500 mb-1">Est. Delivery</div>div><div className="text-sm text-zinc-200">{trackingResult.estDelivery}</div>div></div>div>}
                                                            </div>div>
                                                            <div className="text-xs text-zinc-500 uppercase tracking-widest mb-4 font-medium">Tracking Timeline</div>div>
                                                            <div className="flex flex-col">
                                                              {trackingResult.events && trackingResult.events.map((event, i) => (
                                                                  <div key={i} className="flex gap-4">
                                                                                        ...
                                                                                        <div className={`pb-5 ${event.completed ? '' : 'opacity-40'}`}>
                                                                                                                <div className={`text-sm font-medium ${event.completed ? 'text-white' : 'text-zinc-500'}`}>{event.description}</div>div>
                                                                                                                <div className="text-xs text-zinc-500 mt-0.5">{event.location} · {event.timestamp}</div>div>
                                                                                          </div>div>
                                                                  </div>div>
                                                                ))}
                                                              {(!trackingResult.events || trackingResult.events.length === 0) && (
                                                                  <div className="text-sm text-zinc-500">No tracking events available yet.</div>div>
                                                                              )}
                                                            </div>div>
                                            </div>div>
                                        )}
                              ...
                              {!trackingResult && !trackingLoading && !trackingError && (
                                            <div className="text-center py-16 text-zinc-600">
                                                            ...
                                                            <div className="text-sm">Enter a tracking number to see live status</div>div>
                                            </div>div>
                                        )}
                            </div>div>
                              )}
                  ...
                  {/* Footer */}
                      <footer className="border-t border-border mt-16 py-6">
                              <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                                        <div className="text-xs text-zinc-600">Powered by Parker Express</div>div>
                                        <div className="text-xs text-zinc-600">dashboard.parkerexpress.com.au</div>div>
                              </div>div>
                      </footer>footer>
                </div>div>
              )
